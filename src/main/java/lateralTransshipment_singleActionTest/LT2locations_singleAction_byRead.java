...
public class LT2locations_singleAction_byRead {
	...
	public static double[][] readFromTXT(double[][] array) throws Exception {
		...
		Scanner sc = new Scanner( new BufferedReader(new FileReader("src/main/java/lateralTransshipment/optimalCostToRead2-4.txt")));
...
//		System.out.println(Arrays.deepToString(array));
	...
	public static LTsolution LT2locations_singleAction_byRead(LTinstance instance, boolean noInitialTransship, boolean noInitialOrder,
		...
		double[][] futureCost = new double[Stages-actionPeriodIdx-1][inventoryPairs.length];
		try {
		...
		for(int i=0; i<inventoryPairs.length; i++) {
			optimalCost[actionPeriodIdx*2+2][i] = futureCost[0][i];
		}
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<noInitialTransship.length; i++) {
			for(int k=0; k<R.length; k++) {
				...
				LTsolution solution = LT2locations_singleAction_byRead(instance, noInitialTransship[i], noInitialOrder[i], 
