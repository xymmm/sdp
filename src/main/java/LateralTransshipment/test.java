...
public class test {
	...
	public static void testSingleAction(Instance instance, int[] testInventory, int[] testAction) {
		...
		double futureCost = 0; double immediateCost = 0;
...
//							imHolding[0]+imHolding[1]+imPenalty[0]+imPenalty[1]);
		...
		totalCost /= mass;
		totalHolding[0] /= mass; totalHolding[1] /= mass; totalPenalty[0]/=mass; totalPenalty[1]/=mass;
		
		double ETC = transshippingCost + orderingCost[0] + orderingCost[1] + totalCost;
		...
		System.out.println("ordering cost = " + Arrays.toString(orderingCost));
