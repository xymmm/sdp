...
public class Demand {
   ...
   public static double[] normalProbability(double meanDemand, double stdPara, double tail) {
	   ...
	   double maxDemand =  dist.inverseCumulativeProbability(1-tail);
	   double[] demandProbabilities = new double[(int) (Math.round(maxDemand) + 1)];
	   
	   for(int i=0; i<=maxDemand;i++) {
		   demandProbabilities [i] = (dist.probability(i-0.5, i+0.5))/(dist.cumulativeProbability(maxDemand) - dist.cumulativeProbability(-0.5));
	   }
